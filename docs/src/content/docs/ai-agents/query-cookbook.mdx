...
The Budget plugin stores a **complete category set per month** in `plugin_budget.categories` — there is no template that carries forward, so a month with no rows simply hasn't been set up. The tables exist for everyone (core migrations create them); they only have rows if the user budgets.
