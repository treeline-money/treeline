...
**Transfers aren't spending or income — and tags are entirely user-defined.** Money moving between your own accounts (savings transfers, brokerage moves, credit card payments) shows up as ordinary transactions on *both* sides, and there is **no fixed tag vocabulary**: one user tags these `transfer`, another `savings`, another nothing at all. Before trusting any cash-flow number, discover what *this* user's tags mean:
...
If the biggest rows are account-to-account moves, note which tags they carry — and when in doubt, ask the user. A tell-tale sign of leaking transfers is income that closely tracks spending month to month. If the user doesn't tag internal movement at all, say so rather than reporting silently inflated numbers.

The queries below exclude internal movement with `list_has_any(tags, ['transfer', 'payment'])` — **that list is a placeholder**; substitute the tags this user actually uses. Exclusion must happen **at the row level, before `UNNEST`** — a transfer tagged `['transfer', 'savings']` leaks into per-tag totals through its second tag if you filter after unnesting.
