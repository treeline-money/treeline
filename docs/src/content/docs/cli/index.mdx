...
- `tl schema` - Show database schema (tables, views, columns)
...
- `tl mcp` - Start the MCP server (STDIO) for a same-machine AI agent
- `tl serve` - Run a hub: HTTP server for device push/pull and remote MCP ([Remote Access](/remote-access/))
- `tl hub` - Link this device to a hub and push/pull (`link`, `push`, `pull`, `watch`, `status`, `conflicts`, `resolve`, `unlink`, `tokens`)
- `tl setup` - Set up integrations (SimpleFIN, Lunchflow)
- `tl plugin` - Manage plugins
- `tl update` - Update to the latest version (`--check` to look without installing)
...
Sync with a custom lookback window (default is 7 days for incremental, 90 days for initial):

```bash
tl sync --lookback-days 30
```
