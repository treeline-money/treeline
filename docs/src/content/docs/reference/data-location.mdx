...
tl backup restore treeline-2026-01-15T14-30-22-123456.zip
```

Backup files are ZIP archives named with timestamps: `treeline-{YYYY-MM-DDTHH-MM-SS}-{micros}.zip`

**Recommendation:** Periodically copy backups to external storage or a cloud backup service. Backup archives contain the DuckDB database file plus `settings.json` and `encryption.json` when those exist.
...
These appear only once you run a hub or link a device — see [Remote Access](/remote-access/). On a fully local install, none of them exist.

| File | Where | Purpose |
|------|-------|---------|
| `hub-token` | On the hub | Master token used to authorize new devices and agents |
| `oauth.json` | On the hub | Registered clients and issued access/refresh tokens |
| `hub.json` | On a linked device | Which hub this device is linked to, plus last push/pull times |
| `hub-sync.json` | On a linked device | Replication state used to detect changes and conflicts |
...
:::caution[Avoid Concurrent Access]
Close the desktop app and ensure no CLI commands are running before accessing the database from external tools. This prevents potential conflicts.
:::

:::tip[CLI Works While App Runs]
The CLI (`tl`) can run commands while the desktop app is open. Operations are automatically coordinated, so you can safely run `tl query` or `tl status` without closing the app.
:::
