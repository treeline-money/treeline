...
| `notes` | VARCHAR | Free-form note you attach to the transaction |
...
Canonical, tested queries for common questions — net worth, spending, income, savings rate, recurring charges — live in the [Query Cookbook](/ai-agents/query-cookbook/), along with the conventions (sign handling, transfer exclusion, snapshot semantics) that make naive versions return wrong numbers.
