...
Every linked device and connected agent holds its own scoped token. The tokens live on the Hub and never travel with the bundle, so list and revoke them **on the Hub itself** — the machine running `tl serve`, or a shell into the container:
