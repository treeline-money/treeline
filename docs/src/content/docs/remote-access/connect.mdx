...
**From the desktop app:** Hub support is currently behind an experimental flag — enable **Hub** under Settings → Advanced → Experimental features, then go to Settings → Hub and click **Link a hub**. Enter your Hub's URL and the same browser authorization opens. Once linked, the app syncs automatically while it's running — no `watch` command needed — and shows sync state in the status bar, including conflicts with a **Review & resolve** flow.
...
tl hub conflicts  # inspect sync conflicts (see below)
tl hub resolve    # resolve them by picking a side
